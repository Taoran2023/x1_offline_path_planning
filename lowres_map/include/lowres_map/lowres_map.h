...
struct sch_node{
    ...
    sch_node(shared_ptr<sch_node> &node){
        ...
        h_g_score_ = node->h_g_score_;
        g_score_ = node->g_score_;
    }
    Eigen::Vector3i pos_;
...
// ── Offline Topo Graph ─────────────────────────────────────────
enum EdgeTag { EDGE_GROUND, EDGE_AERIAL, EDGE_CROSS, EDGE_UNKNOWN };
...
struct TopoEdge {
    ...
    EdgeTag                     edge_tag_;
    bool                        is_gate_;
    std::vector<Eigen::Vector3d> path_;  // LR-layer path from→to
};
...
public:
     ...
     * @return false 
     */
    bool BimodelGetPath(const Eigen::Vector3d &start, const Eigen::Vector3d &end, vector<Eigen::Vector3d> &path, bool local = false, int search_num = 500);
     ...
     * @return false 
     */
    bool BimodelGetPath(const Eigen::Vector3d &start, const Eigen::Vector3d &end, list<Eigen::Vector3d> &path, bool local = false, int search_num = 500);
    ...
    bool PrunePath(const list<Eigen::Vector3d> &path, list<Eigen::Vector3d> &pruned_path, double &length, double window);
    ...
    bool BimodelDjkstraLocalDist(const Eigen::Vector3d &pos, list<list<Eigen::Vector3d>> &paths, list<list<Eigen::Vector3d>> &pruned_paths,
                                                                                        list<Eigen::Vector3d> &p_l, list<double> &d_l);
    inline T_TAG GetNodeTag(const Eigen::Vector3d &pos){
        auto node = GetNode(pos);
        if(node == NULL || node == Outnode_) return UNKNOWN;
        return node->tag_;
    }
    ...
    // Bimodel
    void Debug3(list<list<Eigen::Vector3d>> &paths);

    void DebugTie(const Eigen::Vector3d &p);

    inline float GetNodeTraversabilityScore(const Eigen::Vector3i pos);
    inline float GetNodeTraversabilityScoreOffline(const Eigen::Vector3i &pos);
    inline bool IsGroundTraversable(const Eigen::VectorXd &pos, double traversable_thr = 0.8);

    // ── Offline API ────────────────────────────────────────────
    void OfflineInitLowResMap();
    void OfflineBuildTopo();
    void OfflineBuildTopoModAware();
    void CollectLowResTags(std::vector<Eigen::Vector3d> &ground,
                           std::vector<Eigen::Vector3d> &aero,
                           std::vector<Eigen::Vector3d> &trans);

    // Insert a query point (start/goal) into topo_adj_ after graph construction.
    // Searches XY ±2 LR nodes + full Z sweep for the nearest node matching tag.
    // Connects to same-tag neighbors via LR Dijkstra + PrunePath.
    // Returns the topo node id, or UINT32_MAX on failure (err_msg filled if provided).
    uint32_t InsertQueryPoint(const Eigen::Vector3d& pos, T_TAG tag,
                              std::string* err_msg = nullptr);

    // Remove a query point and all its edges from the graph.
    void RemoveQueryPoint(uint32_t node_id);

    std::unordered_map<uint32_t, TopoNode>              topo_nodes_;
    std::unordered_map<uint32_t, std::vector<TopoEdge>> topo_adj_;
    std::unordered_set<uint32_t>                        non_reachable_;
    uint32_t                                            next_topo_id_ = 0;
    double                                              topo_max_range_ = 12.0;
    ...
    list<pair<uint32_t, Eigen::Vector3d>> id_Hpos_dist_;                  // localEigen::Vector3d  H
    ...
    inline Eigen::Vector3i GetBlockIdxFromId(const int& id);
    inline int GetBlockIdFromIdx(const Eigen::Vector3i& idx);
    inline std::shared_ptr<LR_block> GetBlock(const int& id);
...
private:
    ...
    bool BimodelAstar(const Eigen::Vector3d &start, const Eigen::Vector3d &end, vector<Eigen::Vector3d> &path, const int &workid, int search_num, bool local = false);
    bool BimodelAstar(const Eigen::Vector3d &start, const Eigen::Vector3d &end, list<Eigen::Vector3d> &path, const int &workid, int search_num, bool local = false);
    ...
    rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr debug_pub_, bimodel_debug_pub_;
    ...
    // ── Offline params ─────────────────────────────────────────
    std::unordered_map<int, uint32_t> node_topo_id_;   // global LR idx → topo id
    float  ground_thr_       = 0.6f;
    double min_aero_height_  = 0.5;   // world z lower bound; can be AERO only above this value
    double max_aero_height_  = 5.0;   // world z upper bound; AERO if below this and score=0
    double topo_spacing_thr_ = 3.0;
    double topo_sample_prob_ = 0.8;
};
...
inline void LowResMap::RetrieveHPathTemp(const int &start, list<Eigen::Vector3d> &path, double &length){
    ...
    if(node == NULL) {//debug
        RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "error RetrieveHPathTemp1!");
        rclcpp::shutdown();
        return;
    }
    ...
    if(cur_node == NULL || cur_node->status_ != in_close) {//debug
        RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "error RetrieveHPathTemp2!");
        rclcpp::shutdown();
        return;
    }
        ...
        if(tie_it != id_idx_dist_.end()){
            if(tie.in_dir_ == 128){
                ...
                if(PrunePathTempt(path, path_prune, length)){
                    ...
                    RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "error PrunePathTempt!");
                    for(auto &p : path){
...
inline pair<int,int> LowResMap::SetNode(const Eigen::Vector3d &pos, bool get_h){
    ...
    if(GetBlock3Id(pos, blockid)){
        ...
        if(gridBLK_[blkid]->local_grid_[nodeid] == NULL){
            RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "Expanding NULL NODE %d %d!!!", blkid, nodeid);
        }
        ...
        // debug tag_
        // if(nodeid == 1){
        //     gridBLK_[blkid]->local_grid_[nodeid]->tag_ = AERO;
        // }
        // else    gridBLK_[blkid]->local_grid_[nodeid]->tag_ = AERO;
        ...
        float score = GetNodeTraversabilityScore(node3i);
        // if(score>0){
        // cout<<"LosRes Score "<<score<<endl;
        // }
        if(score>0.8){  //hardcoded 0.8
            gridBLK_[blkid]->local_grid_[nodeid]->tag_ = GROUND;
            gridBLK_[blkid]->local_grid_[nodeid]->t_score_ = score;
...
inline void LowResMap::SetEXPNode(const Eigen::Vector3d &pos){
    ...
    if(GetBlock3Id(pos, blockid)){
        ...
        int nodeid = GetNodeId(pos, gridBLK_[blkid]);
        if(gridBLK_[blkid]->local_grid_[nodeid] != NULL){
            RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "Expanding expanded NODE %d %d!!!", blkid, nodeid);
        }
...
inline void LowResMap::SetTopoNode(const Eigen::Vector3d &pos, shared_ptr<LR_node> &lr_node, shared_ptr<sch_node> &node){
    ...
    if(GetBlock3Id(pos, blockid)){
        ...
        int nodeid = GetNodeId(pos, gridBLK_[blkid]);
        if(gridBLK_[blkid]->local_grid_[nodeid] != NULL){
            RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "Expanding NULL TOPO NODE %d %d!!!", blkid, nodeid);
        }
        else{
            gridBLK_[blkid]->local_grid_[nodeid] = make_shared<LR_node>();
...
inline void LowResMap::SetEXPNode(const int &b_id, const int &n_id, const Eigen::Vector3i &bk3i){
    if(b_id >= gridBLK_.size()){//debug
        ...
        RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "SetEXPNode(const int &b_id, const int &n_id, const Eigen::Vector3i &bk3i)0");
        return;
    }
    if(gridBLK_[b_id] == NULL){
    ...
    if(n_id >= gridBLK_[b_id]->local_grid_.size()){//debug
        ...
        RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "SetEXPNode(const int &b_id, const int &n_id, const Eigen::Vector3i &bk3i)1");
        return;
    }
    //set node
    if(gridBLK_[b_id]->local_grid_[n_id] != NULL){
        RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "Expanding expanded NODE %d %d!!!", b_id, n_id);
    }
...
inline void LowResMap::UpdateTie(const int &bid, const int &nid, const uint32_t &rid, bool new_h){
            ...
            // if( (!new_h && tie.dist_ < s_n->h_g_score_) || (tie.dist_ < s_n->g_score_ && new_h)){
                ...
                if((s_n->h_parent_ == NULL && !new_h) || (s_n->parent_ == NULL && new_h)){
                   tie.in_dir_ = 128; 
                    ...
                    else RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "error dir tie");
    ...
    if(new_tie){
        ...
        if((s_n->h_parent_ == NULL && !new_h) || (s_n->parent_ == NULL && new_h)){
            ...
            else RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "error dir tie2");
...
//  check local missing
// inline bool LowResMap::HaveLocalOccupiedNeighbour(const Eigen::Vector3i &pos){
//     Eigen::Vector3i npos = pos;
//     int nodeid;
//     shared_ptr<LR_node> node;
//     for(int i = 0; i < 3; i++){
...
inline float LowResMap::GetNodeTraversabilityScore(const Eigen::Vector3i pos){
    Eigen::Vector3d chk_pos;
    Eigen::Vector3i neighbour_pos = pos;
    int occ_num= 0;
    float score = 0.0;
    ...
    for(int dx = -1; dx<=1; dx++){
        for(int dy = -1; dy<=1; dy++){
            int dz = -1;
            Eigen::Vector3i v(dx, dy, dz);
            neighbour_pos = pos+v;
            // for(int i=0; i<3; i++){
                // if(neighbour_pos[i]<0){
                //     return -2.0;
                // }
            // }

            Eigen::Vector3d startpos = neighbour_pos.cast<double>().cwiseProduct(node_scale_) + origin_;
            ...
            for(int x = 0; x < node_size_(0); x++)
                ...
                VoxelState cstatus = map_->GetVoxState(chk_pos);
                if(cstatus == VoxelState::occupied){
                    float temp_score = map_->GetVoxScore(chk_pos);
                    if(temp_score > 0){
                        score += temp_score;
                        occ_num ++;
                        // cout<<"success!! "<<score <<" num "<<occ_num<<endl;
                    }
...
inline float LowResMap::GetNodeTraversabilityScoreOffline(const Eigen::Vector3i &pos) {
    Eigen::Vector3d center = IdtoPos(pos);
    const double step   = map_->resolution_;          // BM voxel step
    const double depth  = node_scale_.z();      // max downward distance
    const double radius = node_scale_.z() * 0.5;

    // Step 1: downward ray to find first occupied BM voxel
    Eigen::Vector3d hit_pos;
    bool found = false;
    for (double dz = step; dz <= depth + 1e-6; dz += step) {
        Eigen::Vector3d chk(center.x(), center.y(), center.z() - dz);
        if (map_->GetVoxState(chk) == VoxelState::occupied) {
            hit_pos = chk;
            found = true;
            break;
        }
    ...
    // Step 2: average traversability score within radius around hit point
    ...
    for (double dx = -radius; dx <= radius + 1e-6; dx += step)
        for (double dy = -radius; dy <= radius + 1e-6; dy += step)
            for (double dz = -radius; dz <= radius + 1e-6; dz += step) {
                Eigen::Vector3d p = hit_pos + Eigen::Vector3d(dx, dy, dz);
                if (map_->GetVoxState(p) == VoxelState::occupied) {
                    float s = map_->GetVoxScore(p);
                    if (s > 0.0f) { score_sum += s; count++; }
                }
            }

    return count > 0 ? score_sum / float(count) : 0.0f;
}

inline bool LowResMap::IsGroundTraversable(const Eigen::VectorXd &pos, double traversable_thr){
    Eigen::Vector3d p(pos(0), pos(1), pos(2));
    int blockid = GetBlockId(p);
    if(blockid != -1){
        shared_ptr<LR_block> fp_ptr = gridBLK_[blockid];
        if(fp_ptr != NULL){
            shared_ptr<LR_node> node = GetNode(p, fp_ptr);
            if(node != NULL && node != Outnode_ && !node->flags_[0] && node->tag_== GROUND && node->t_score_>traversable_thr) return true;
            else return false;
        }
        else{
            return false;
...
// id to Eigen_ID
inline Eigen::Vector3i LowResMap::GetBlockIdxFromId(const int& id)
{
    Eigen::Vector3i idx(-1, -1, -1);

    if (id < 0) return idx;

    // IMPORTANT:
    // Must match your GetBlockId() convention exactly.
    idx.z() = id / b_n_(1);
    int rem = id % b_n_(1);
    idx.y() = rem / b_n_(0);
    idx.x() = rem % b_n_(0);
...
// Eigen_ID to id
inline int LowResMap::GetBlockIdFromIdx(const Eigen::Vector3i& idx)
{
    if (idx.x() < 0 || idx.x() >= b_n_(0) ||
        idx.y() < 0 || idx.y() >= b_n_(1) ||
        idx.z() < 0 || idx.z() >= b_n_(2))
    {
        return -1;
    }

    return idx.z() * b_n_(1) + idx.y() * b_n_(0) + idx.x();
}

inline std::shared_ptr<LR_block> LowResMap::GetBlock(const int& id)
{
    if (id < 0 || id >= gridBLK_.size())
        return nullptr;
