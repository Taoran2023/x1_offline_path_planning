...
using namespace std;
namespace lowres{
bool LowResMap::BimodelGetPath(const Eigen::Vector3d &start, const Eigen::Vector3d &end, vector<Eigen::Vector3d> &path, bool local, int search_num){
    int workid;
    // ros::WallTime startT = ros::WallTime::now();
    while(!GetWorker(workid)){
        std::this_thread::sleep_for(std::chrono::nanoseconds(10000));
    }
    // RCLCPP_WARN(rclcpp::get_logger("lowres_map"), "start a*");
    //  std::cout << "\033[0;34m GetWorker: \033[0m"<<(ros::WallTime::now() - startT).toSec() << std::endl;
    return BimodelAstar(start, end, path, workid, search_num, local);
    
}

bool LowResMap::BimodelGetPath(const Eigen::Vector3d &start, const Eigen::Vector3d &end, list<Eigen::Vector3d> &path, bool local, int search_num){
    int workid;
    // ros::WallTime startT = ros::WallTime::now();
    while(!GetWorker(workid)){
        std::this_thread::sleep_for(std::chrono::nanoseconds(10000));
...
bool LowResMap::BimodelAstar(const Eigen::Vector3d &start, const Eigen::Vector3d &end, vector<Eigen::Vector3d> &path, const int &workid, int search_num, bool local){
    //check two points
    ...
    if(local){
        if(!IsLocalFeasible(start) || !IsLocalFeasible(end)){
            ...
            RCLCPP_WARN(rclcpp::get_logger("lowres_map"), "error IsLocalFeasible");
            return false;
        }
    }
    else{
        if(!IsFeasible(start) || !IsFeasible(end)){
            ...
            RCLCPP_WARN(rclcpp::get_logger("lowres_map"), "error IsFeasible");
            return false;
        }
    }
    ...
    if(lr_node == NULL || lr_node == Outnode_) RCLCPP_WARN(rclcpp::get_logger("lowres_map"), "ERROR Astar"); //debug
    lr_node->topo_sch_ = c_node;
    open_set_[workid].push(c_node);
    int search_iter = 0;

    // shared_ptr<LR_node> start_lr_node;
    // shared_ptr<LR_node> end_lr_node;
    // start_lr_node = GetNode(start);
    // end_lr_node = GetNode(end);
    // double v_cost = 0.0;
    // if(start_lr_node->tag_ == AERO && end_lr_node->tag_ == GROUND){
    //     v_cost = 100;
    // }
    // else if(start_lr_node->tag_ == GROUND && end_lr_node->tag_ == AERO){
    //     v_cost = -100;
    // }
        ...
        for(int dim = 0; dim < 3; dim++){
            ...
            for(int off = -1; off <= 1; off += 2){
                ...
                if(c_node == NULL){                         //create a new node
                    ...
                    const double ground_cost = 1.0;
                    const double aerial_cost = 50.0;
                    const double trans_cost  = 1.0;
                    shared_ptr<LR_node> ep_lr_node = GetNode(IdtoPos(ep_node->pos_));
                    double step_cost;
                    if(ep_lr_node->tag_ == lr_node->tag_){
                        step_cost = (ep_lr_node->tag_ == GROUND ? ground_cost : aerial_cost)
                                    * GetDist(diff(0), diff(1), diff(2));
                    }
                    else if(ep_lr_node->tag_ == GROUND && lr_node->tag_ == AERO){
                        // takeoff: heavy penalty
                        step_cost = trans_cost + aerial_cost * GetDist(diff(0), diff(1), diff(2));
                    }
                    else{
                        // landing (AERO->GROUND): light penalty
                        step_cost = trans_cost + ground_cost * GetDist(diff(0), diff(1), diff(2));
                    }
                    c_node->g_score_ = ep_node->g_score_ + step_cost;
                    ...
                    double g_tmp = ep_node->g_score_ + step_cost;
    ...
    if(search_iter >= search_num){
        ...
        RCLCPP_WARN(rclcpp::get_logger("lowres_map"), "too long");
    }
    else{
        RCLCPP_WARN(rclcpp::get_logger("lowres_map"), "empty!");
    }
    FreeWorker(workid, node_list);
    return false;
}
...
bool LowResMap::BimodelAstar(const Eigen::Vector3d &start, const Eigen::Vector3d &end, list<Eigen::Vector3d> &path, const int &workid, int search_num, bool local){
    //check two points
    vector<shared_ptr<sch_node>> node_list; //to maintain nodes  
    if(local){
        if(!IsLocalFeasible(start) || !IsLocalFeasible(end)){
...
//     // standard Dijskra Search, set search range, set lr_node->topo_sch_ 
//     while(!pr_l.empty()){
//         c_node = pr_l.top();
//         pr_l.pop();
//         if(c_node->status_ == in_close) continue;
...
//     // after Dijskra, check (p:pl  targets) lr_node->topo_sch_, whether in range
//     for(auto &p : p_l){
//         lr_node = GetNode(p);
//         // node doesnt exist, Unode, Outnode_: outside lowresmap
...
//         if(!IsFeasible(p)) RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "error end feasible!");
//         // sparse and straight path
...
//         }
//         else{
//             RCLCPP_WARN(rclcpp::get_logger("lowres_map"), "fail prune????? how?");
...
//         RCLCPP_ERROR(rclcpp::get_logger("lowres_map"), "DjkstraLocal0");
//         rclcpp::shutdown();
//     }
...
//             Eigen::Vector3i diff(0, 0, 0);
//             for(int off = -1; off <= 1; off += 2){
//                 // RCLCPP_WARN(rclcpp::get_logger("lowres_map"), "DjkstraLocal4");
//                 diff(dim) = off;
//                 pos = IdtoPos(diff+c_node->pos_);
//                 lr_node = GetNode(pos);
...
//             RCLCPP_WARN(rclcpp::get_logger("lowres_map"), "ERROR FreeWorker");
//         }
//     }
...
bool LowResMap::BimodelDjkstraLocalDist(const Eigen::Vector3d &start,
        list<list<Eigen::Vector3d>> &paths, list<list<Eigen::Vector3d>> &pruned_paths,
    ...
    lr_node = GetNode(start);
    if(lr_node == NULL || lr_node == Outnode_ || lr_node->flags_[0]) return false;
    ...
    // mode-aware edge cost helper
    auto bimodalEdgeCost = [&](shared_ptr<LR_node> from_lr, shared_ptr<LR_node> to_lr,
                                const Eigen::Vector3i &d) -> double {
        const double ground_cost = 1.0;
        const double aerial_cost = 50.0;
        const double trans_cost  = 1.0;
        double dist = GetDist(d(0), d(1), d(2));
        if(from_lr->tag_ == to_lr->tag_){
            return (from_lr->tag_ == GROUND ? ground_cost : aerial_cost) * dist;
        } else if(from_lr->tag_ == GROUND && to_lr->tag_ == AERO){
            return trans_cost + aerial_cost * dist;  // takeoff: heavy
        } else {
            return trans_cost + ground_cost * dist;  // landing: light
        }
    };
    ...
    while(!pr_l.empty()){
        ...
        shared_ptr<LR_node> c_lr = GetNode(IdtoPos(c_node->pos_));
        ...
        for(int dim = 0; dim < 3; dim++){
            ...
            for(int off = -1; off <= 1; off += 2){
                ...
                pos = IdtoPos(diff + c_node->pos_);
                lr_node = GetNode(pos);
                if((pos - start).norm() > eu_range_) continue;
                if(lr_node == NULL || lr_node->flags_[0] || lr_node == Outnode_) continue;

                double edge_g = bimodalEdgeCost(c_lr, lr_node, diff);
                double g = c_node->g_score_ + edge_g;
                if(g > max_g_cost_) continue;
                ...
                double g_eps = c_node->g_score_ + edge_g * 1.0001;
                if(n_node->status_ == in_open && g_eps < n_node->g_score_){
                    n_node->status_ = in_close;
                    lr_node->topo_sch_ = make_shared<sch_node>();
                    lr_node->topo_sch_->status_ = in_open;
                    lr_node->topo_sch_->pos_ = c_node->pos_ + diff;
    ...
    // retrieve paths for all targets
    for(auto &p : p_l){
        lr_node = GetNode(p);
        if(lr_node == NULL || lr_node->flags_[0] || lr_node == Outnode_){
            paths.push_back(empty_path);
...
void LowResMap::Debug3(list<list<Eigen::Vector3d>> &paths){
