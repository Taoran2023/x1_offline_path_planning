...
public:
    ...
    /**
     * @brief Get the Vox traversability score
     * 
     * @param id 
     * @return score 
     */
    inline float GetVoxScore(const int &id);
     ...
     * @return score 
     */
    inline float GetVoxScore(const Eigen::Vector3i &id);
     ...
     * @return score 
     */
    inline float GetVoxScore(const Eigen::Vector3d &id);
     ...
     * @param pcl 
     */
    void InsertPcl(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &pcl);

    // Offline batch insert: all points inside map bounds → occupied. No raycasting.
    void OfflineInsertPcl(const pcl::PointCloud<pcl::PointXYZ>::Ptr &pcl);

    // Traverse all MIXED blocks and collect occupied/free voxel world positions + colors.
    void CollectVoxels(std::vector<Eigen::Vector3d> &occ_pts,
                       std::vector<std_msgs::msg::ColorRGBA> &occ_colors,
                       std::vector<Eigen::Vector3d> &free_pts);

    // Collect surface voxels with valid traversability scores (requires traversability_compute_=true).
    void CollectTraversability(std::vector<Eigen::Vector3d> &pts,
                               std::vector<float> &scores);

    // After offline insert: mark all UNKNOWN voxels/blocks as free.
    // UNKNOWN blocks → state = FREE (zero allocation).
    // MIXED blocks   → unobserved voxels set to thr_min_ (minimum free log-odds).
    void OfflineFillUnknownFree();

    // BFS wave propagation: fill is_surface voxels that lack a traversability score
    // (caused by sparse point clouds). Only touches voxels already marked is_surface.
    // fill_radius: neighbor search radius in voxels (default=1).
    void PatchTraversabilityMap(int fill_radius = 1);

    inline std_msgs::msg::ColorRGBA Getcolor(const double z);
     ...
     * @param depth
     */
    void InsertImg(const sensor_msgs::msg::Image::ConstSharedPtr &depth);
    ...
    // traversability
    NormalSmoothParams       nsp_;   // normal smoothing params
    TraversabilityScoreParams tsp_;  // score computation params
    float              occ_thr_log_;      // occupancy threshold (not using)
    float              voxel_size_;       // = resolution_ (not using)
    Eigen::Vector3f    gravity_dir_unit_; // (0,0,1) or from IMU

private:
    //callback func
    void InsertPCLCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &pcl);
    void InsertDepthCallback(const sensor_msgs::msg::Image::ConstSharedPtr &img);
    void CamParamCallback(const sensor_msgs::msg::CameraInfo::ConstSharedPtr &param);

    //Timer
    void ShowMapCallback();
    void SwarmMapCallback();
    void StatisticV();

    void ProjectToImg(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &pcl, vector<double> &depth_img);
    ...
    rclcpp::Subscription<sensor_msgs::msg::CameraInfo>::SharedPtr camparam_sub_;
    rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr vox_pub_;
    rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr debug_pub_;
    rclcpp::TimerBase::SharedPtr show_timer_, debug_timer_;
    vector<std_msgs::msg::ColorRGBA> color_list_;
    double colorhsize_;
    ...
    //statistic
    bool stat_;
    rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr statistic_pub_;
    rclcpp::TimerBase::SharedPtr statistic_timer_;
    ...
    bool traversability_compute_;
    bool traversability_vis_ ;
...
inline void BlockMap::Flags2Vox(vector<uint8_t> &flags, Eigen::Vector3d &up, Eigen::Vector3d &down,
    ...
    if(flag_num < vox_num && block_state == 3){
        RCLCPP_ERROR(rclcpp::get_logger("block_map"), "error Flags2Vox %d < %d, state: %d", flag_num, vox_num, block_state);
        return;
    }
    else if(block_state == 0){
        RCLCPP_ERROR(rclcpp::get_logger("block_map"), "error Flags2Vox state: 0");
        return;
    }
...
inline void BlockMap::Vox2Msg(exp_comm_msgs::MapC &msg, const uint16_t &f_id, const uint8_t &b_id){
    ...
    for(it(2) = down(2); it(2) < up(2); it(2) += resolution_)
        ...
        if(cur_vs == VoxelState::out){
            RCLCPP_ERROR(rclcpp::get_logger("block_map"), "Vox2Msg how out?");
            rclcpp::shutdown();
            return;
        }
...
inline void BlockMap::UpdateSBS(const int &SBS_id, const int &sub_id){
    ...
    for(it(0) = down(0); it(0) < up(0); it(0) += resolution_)
        ...
        if(vs == out){
            RCLCPP_ERROR(rclcpp::get_logger("block_map"), "UpdateSBS how out?");
            rclcpp::shutdown();
            return;
        }
        else if(vs == unknown){
...
inline VoxelState BlockMap::GetVoxState(const Eigen::Vector3d &pos){
    ...
    if(blockid != -1){
        shared_ptr<Grid_Block> GB_ptr = GBS_[blockid];
        // seems like GB_ptr->state_ can only be UNKNOW or MIX
        // cout<<"state "<<GB_ptr->state_<<" state check!!!!!!!!!! "<<endl;
        if(GB_ptr->state_ == MIXED){
            float odds = GBS_[blockid]->odds_log_[GetVoxId(pos, GBS_[blockid])];
...
// return voxel travers
inline float BlockMap::GetVoxScore(const int &id){
    Eigen::Vector3d pos = IdtoPos(id);
    ...
    if(blockid == -1) {
        ...
        float odds = GBS_[blockid]->odds_log_[GetVoxId(pos, GBS_[blockid])];
        if(odds > 0) return GBS_[blockid]->score_[GetVoxId(pos, GBS_[blockid])];
        // else if(odds < 0 && odds >= thr_min_) return VoxelState::free;
        else return -1.0;
    } 
    // else if(GBS_[blockid]->state_ == GBSTATE::FREE){
...
inline float BlockMap::GetVoxScore(const Eigen::Vector3i &id){
    int voxid = id(0) + id(1) * voxel_num_(0) + id(2) * voxel_num_(0) * voxel_num_(1);
    return GetVoxScore(voxid);
}

inline float BlockMap::GetVoxScore(const Eigen::Vector3d &pos){
    int blockid = GetBlockId(pos);
    if(blockid != -1){
        shared_ptr<Grid_Block> GB_ptr = GBS_[blockid];
        // seems like GB_ptr->state_ can only be UNKNOW or MIX
        ...
        if(GB_ptr->state_ == MIXED){
            ...
            // else if(odds < 0 && odds > thr_min_ - 1e-3) return 1.0;
            else return -1.0;
        }
        // else if(GBS_[blockid]->state_ == GBSTATE::FREE){
        //     return 1.0;
...
inline std_msgs::msg::ColorRGBA BlockMap::Getcolor(const double z){
    ...
    if(difz > map_upbd_(2)){
        ...
        int hieghtf = floor(difz / (colorhsize_+0.01));
