...
public: 
    ...
    /**
     * @brief Get pose of a ground viewpoint (stub — yaw toward frontier center)
     * TODO: replace with proper ground yaw computation in FrontierGrid
     * @param g_vp_pos  ground VP position (from local_gvps_)
     * @param f_idx     frontier index (for center direction)
     * @param g_vp_pose output pose (xyz + yaw)
     */
    inline bool GetGvpPose(const Eigen::Vector3d &g_vp_pos, const int &f_idx, Eigen::Vector4d &g_vp_pose);
...
private:
    ...
    void LoadVpLines(visualization_msgs::msg::Marker &mk, Eigen::Vector4d &vp);
    inline int Pos2DirIdx(const Eigen::Vector3d &pos, const Eigen::Vector3d &center);
    void SampleVpsCallback();
    void LazySampleCallback();
    void ShowVpsCallback();
    ...
    int scan_count_;
    rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr show_pub_;
    rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr debug_pub_;
    rclcpp::TimerBase::SharedPtr sample_timer_, show_timer_, lazy_samp_timer_;
    //FOV down sample
    ...
    Eigen::Vector3d lowres_node_scale_,lowres_block_scale_;
    Eigen::Vector3i lowres_block_size_, sample_max_gridBLK_ids_,sample_min_gridBLK_ids_;
    double samp_g_h_dir_scale_, samp_g_v_dir_scale_, samp_g_h_dist_max_scale_,samp_g_v_dist_max_scale_, samp_g_h_dist_min_scale_,samp_g_v_dist_min_scale_;
};
...
inline bool FrontierGrid::GetGvpPose(const Eigen::Vector3d &g_vp_pos, const int &f_idx, Eigen::Vector4d &g_vp_pose){
    if(f_idx < 0 || f_idx >= (int)f_grid_.size()) return false;
    if(f_grid_[f_idx].f_state_ != 1) return false;
    g_vp_pose.head<3>() = g_vp_pos;
    // yaw: face the frontier center from the ground VP position
    Eigen::Vector3d dir = f_grid_[f_idx].center_ - g_vp_pos;
    g_vp_pose(3) = atan2(dir(1), dir(0));
    return true;
}
